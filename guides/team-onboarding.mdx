...
## Add AI authorship to onboarding

Install the [Autter CLI](/cli/install) on each developer machine. It records line-level attribution without changing the team's commit workflow.
...
Connected mode can aggregate attribution across the team. Review [Data and privacy](/cli/data-and-privacy) before enabling prompt storage.
...
<Steps>
  ...
  <Step title="Install the CLI">
    Follow [Install the CLI](/cli/install) on the developer's machine, then restart their coding agent.
  </Step>
  <Step title="Choose a storage mode">
    Use local-only mode or connect the machine to your organization. Keep prompts local when required by policy.
