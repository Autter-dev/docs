...
<Tabs>
  ...
  <Tab title="npm">
    ...
    The npm package is a thin bootstrapper: it downloads the same release binary into `~/.autter/bin` and verifies its checksum, so integrations and background-service updates work identically to the script installs. `npx autter onboard` bootstraps without a global install.
  </Tab>
</Tabs>
